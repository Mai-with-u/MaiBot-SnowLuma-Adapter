...
- 修复语音消息：`get_record` 已按 `out_format` 返回服务端转码结果（`base64`）时优先使用该结果，不再重复下载原始 SILK 并退回本地转码；此前在高负载下本地转码超时会把语音降级为 `[voice]` 文本占位，导致本体 ASR 收不到音频。
- 本地 Silk 兜底：解码阶段增加超时保护，解码异常不再被吞掉。
- 本地 Silk 兜底：识别并归一化 `0x03#!SILK_V3` 容器头（QQ AI 语音），与 SnowLuma 服务端行为一致。
- 本地 Silk 兜底单阶段超时由 15s 调整为 30s，高负载机器上 15s 容易误杀。
- `get_record` 失败时记录服务端返回的失败原因，且同一条语音不再重复发起 `get_record`。
- 动作响应内联 `base64` 增加体积上限（16MB），超限跳过内联数据并告警。
- Silk 解码迁移到专用线程池（默认 2 线程），避免无法取消的解码线程拖累插件内其它线程任务。
